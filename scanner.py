...
def create_scanner(args):
    ...
    创建扫描器(统一三层架构 - 分层 AC)
    """
    # 支持 .json 或 .json.gz (压缩版节省发布体积)
    rules_file = Path(__file__).parent / 'rules' / 'dist' / 'all_rules.json'
    if not rules_file.exists():
        rules_file_gz = Path(__file__).parent / 'rules' / 'dist' / 'all_rules.json.gz'
        if rules_file_gz.exists():
            rules_file = rules_file_gz
...
def main():
    ...
    parser.add_argument('--extensions', type=str, default='.py,.python,.js,.javascript,.jsx,.ts,.tsx,.sh,.bash,.ps1,.vbs,.bat,.cmd,.yaml,.yml,.json,.go,.rb,.php,.java,.c,.cpp,.h,.hpp',
