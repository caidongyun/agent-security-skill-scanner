#!/usr/bin/env python3
"""
优化 Resource Exhaustion YARA 规则
提升检测率至 95%+
"""
...
def save_rules():
    """保存优化后的规则"""
    rules_file = Path(RULES_DIR) / "resource_exhaustion_optimized.yar"
    
    with open(rules_file, 'w') as f:
        f.write("// Resource Exhaustion Detection Rules - Optimized\n")
        f.write(f"// Created: 2026-03-30\n")
        f.write("// Target: 95%+ detection rate\n\n")
        f.write(optimized_rules)
    
    print(f"✅ 优化规则已保存：{rules_file}")
    return rules_file
