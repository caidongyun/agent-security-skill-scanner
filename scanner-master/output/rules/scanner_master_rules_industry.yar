...
rule Malicious_Data_Exfiltration {
    ...
    strings:
        $http_post = /requests\.post\s*\([^)]*(password|secret|token|credential|api_key)/
        $exfil_url = /https?:\/\/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/
        $socket_exfil = /socket\.connect\s*\([^)]*\d+\.\d+\.\d+\.\d+/
    condition:
        any of them
}
rule Malicious_Credential_Theft {
    ...
    strings:
        $ssh_key = /\.ssh\/id_rsa/
        $cred_dump = /\b(dump|steal|exfil|leak)\b/
        $cred_send = /requests\.post\s*\([^)]*(password|secret|token)/
    condition:
        $ssh_key or $cred_dump or $cred_send
}
...
rule CRED_EnvVarTheft {
    ...
    strings:
        ...
        $proc = "process.env"
        $osenv = "os.environ"
        $exfil = /requests\.post|socket\.connect|urllib\.request/
    condition:
        ($proc or $osenv) and ($aws or $azure or $gcp) and $exfil
}
