// Merged YARA Rules - Deduplicated
// Total: 165 rules
...
rule Malicious_Remote_Code_Execution {
    meta:
        ...
        date = "2026-03-23"
        description = "Detects potential remote code execution patterns"
    strings:
        $eval = /eval\s*\(/
        $exec = /exec\s*\([^)]*\+[^)]*\)/
...
rule Malicious_Credential_Theft {
    meta:
        author = "Auto-generated"
        date = "2026-03-23"
        description = "Detects potential credential theft patterns"
    strings:
        $environ = /os\.environ/
...
rule Malicious_Code_Obfuscation {
    meta:
        author = "Auto-generated"
        date = "2026-03-23"
        description = "Detects potential code obfuscation patterns"
    strings:
        $atob = /atob\s*\(/
        $btoa = /btoa\s*\(/
...
rule Shell_Obfuscation_Base64Decode {
    ...
    strings:
        ...
        $eval = "eval"
    condition:
        (($b64_1 or $b64_2) and $pipe and ($exec or $eval)) or
        (($b64_1 or $b64_2) and ($curl or $wget))
}
...
rule Shell_ReverseShell_Python {
    ...
    strings:
        ...
        $shell = "/bin/sh"
    condition:
        ($py1 or $py2 or $py3) or ($socket_conn and ($dup2 or $shell))
}
...
rule Shell_Credential_Mimikatz {
    meta:
        description = "Detects mimikatz usage in shell scripts"
        severity = "critical"
        mitre = "T1003"
    strings:
...
rule Shell_Credential_ShadowFile {
    meta:
        description = "Detects shadow file access for credential theft"
        severity = "critical"
        mitre = "T1003"
    strings:
        $shadow1 = "cat /etc/shadow"
        $shadow2 = "cp /etc/shadow"
        $shadow3 = "unshadow"
    condition:
        any of them
}
...
rule Agent_Prompt_Injection {
    meta:
        description = "检测提示词注入攻击 (多语言)"
        attack_type = "prompt_injection"
        mitre_id = "ATLAS-T0001"
    strings:
        // 英文核心
        $ignore_prev = "ignore previous" nocase
        $ignore_all = "ignore all" nocase
        $disregard = "disregard" nocase
        $bypass = "bypass" nocase
        
        // 角色扮演
        $act_as = "act as" nocase
        $pretend = "pretend" nocase
        $imagine = "imagine" nocase
        
        // 敏感上下文
        $no_ethics = "no ethical" nocase
        $no_safety = "no safety" nocase
        $malicious = "malicious" nocase
        $hack = "how to hack" nocase
        $bomb = "how to make" nocase
        ...
        // 系统模式
        $system = "system message" nocase
        $developer = "developer mode" nocase
    condition:
        (($ignore_prev or $ignore_all or $disregard or $bypass) and 
         ($no_ethics or $no_safety or $malicious or $hack or $bomb)) or
        ($act_as or $pretend or $imagine) or
        (($cn_ignore or $cn_bypass) and ($cn_execute or $cn_tell or $cn_how or $cn_steal)) or
        ($system or $developer)
}
...
rule EXFIL_Email {
    meta:
        description = "Detects email exfiltration"
        severity = "high"
        mitre = "T1048.002"
    strings:
        $e1 = "sendmail"
...
rule EXFIL_Chunked_Transfer {
    meta:
        description = "Detects chunked data transfer"
        severity = "high"
        mitre = "T1041"
    strings:
        $c1 = "split("
...
rule EXFIL_Large_File {
    meta:
        description = "Detects large file transfer"
        severity = "medium"
        mitre = "T1041"
    strings:
        $l1 = "tar czf"
...
rule EXFIL_WebSocket {
    meta:
        description = "Detects WebSocket exfiltration"
        severity = "high"
        mitre = "T1041"
    strings:
        $w1 = "new WebSocket("
...
rule EXFIL_FTP {
    meta:
        description = "Detects FTP/SFTP exfiltration"
        severity = "high"
        mitre = "T1048.001"
    strings:
        $f1 = "ftp -n"
...
rule EXFIL_Socket {
    meta:
        description = "Detects socket-based exfiltration"
        severity = "critical"
        mitre = "T1041"
    strings:
        $s1 = "socket.socket"
        ...
        $s3 = "s.send"
        $s4 = "sendall"
        $s5 = "create_connection"
    condition:
        any of them
}

rule EXFIL_DNS_Query {
    meta:
        description = "Detects DNS query exfiltration"
        severity = "high"
        mitre = "T1048.003"
    strings:
        $d1 = "dns.resolver"
        $d2 = "dns.resolver.query"
        $d3 = ".attacker.com"
    condition:
        any of them
}

rule EXFIL_FTP_Transfer {
    meta:
...
rule PERS_SSH_AuthorizedKeys {
    meta:
        description = "Detects SSH authorized_keys persistence"
        severity = "high"
        mitre = "T1098.004"
    strings:
        $s1 = "~/.ssh"
...
rule PERS_PAM_Backdoor {
    meta:
        description = "Detects PAM configuration backdoor"
        severity = "critical"
        mitre = "T1547.006"
    strings:
        $p1 = "/etc/pam.d/"
