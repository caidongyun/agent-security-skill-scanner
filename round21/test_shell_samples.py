#!/usr/bin/env python3
"""
Round 21 - Shell 样本批量测试

验证 Shell 分析器对所有样本的检测效果
"""
...
def test_samples():
    ...
    malicious_dir = base_dir / "shell_malicious"
    safe_dir = base_dir / "shell_safe"
    
    print("=" * 60)
    print("🔍 Round 21 - Shell 样本批量测试")
    print("=" * 60)
    ...
    if malicious_dir.exists():
        for sh_file in sorted(malicious_dir.glob("*.sh")):
            result = analyzer.analyze(str(sh_file))
            malicious_total += 1
            
            # 提取类别
            parts = sh_file.stem.split('_')
    ...
    if safe_dir.exists():
        for sh_file in sorted(safe_dir.glob("*.sh")):
            result = analyzer.analyze(str(sh_file))
            safe_total += 1
